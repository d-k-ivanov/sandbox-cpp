...
int main()
{
    ...
    UString ttest = _ctest;
    u.test("cstring ctor", ttest.length() == 12);

    puts(ttest);

    UString other = std::move(ttest);
    ...
    u.test("substr", ttest.substr(10, 3) == UString("str"));
    u.test("charfind", ttest.char_find('s') == 3);
    u.test("charfind (not found)", ttest.char_find('z') == -1);

    UString string_upper = ttest.upper();
    UString string_lower = string_upper.lower();
    u.test("upper and lower", string_upper == UString("THIS IS A STRING") && string_lower == UString("this is a string"));
